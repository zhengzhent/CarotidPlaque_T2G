...
def svm2pkl(source, save_path):
    ...
    X_test, y_test = load_svmlight_file(os.path.join(source, 'test.py'))
    ...
    pd.concat([y_test, X_test], axis=1).T.reset_index(drop=True).T.to_pickle(os.path.join(save_path, 'test.py.pkl'))
