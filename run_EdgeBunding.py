...
# 读取csv数据
df = pd.read_csv('F:\A_MyWork\Research\CarotidPlaqueVASystem\PlaqueData\Temp3.csv',encoding='gbk')
...
# nodes = [namelist[i+1] for i in range(555)]
# print(nodes)

nodes = []
for i in range(555):
    for j in range(555):
        if adj_matrix[i,j] > 0.02 :
            nodes.append(namelist[i+1])
print(nodes)
# edges = []
# for i in range(555):
#     for j in range(555):
...
with open('Temp_data_0.02.json','w') as f:
    f.write(json_string)