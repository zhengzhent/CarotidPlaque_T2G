...
#
# AUC_FOR_BINCLASS = False #选择不计算AUC
#
#
# def get_training_args():
#     MODEL_CARDS = [x.__name__ for x in IMPLEMENTED_MODELS]
#     parser = argparse.ArgumentParser()  #创建一个解析器对象
#     parser.add_argument("--output", type=str, default='results/')  #输出目录
#     parser.add_argument("--dataset", type=str, required=True, choices=DATASETS)  #数据集
#     parser.add_argument("--normalization", type=str, default='quantile')  #标准化参数
#     parser.add_argument("--model", type=str, default='T2GFormer', choices=MODEL_CARDS)  #模型选择
#     parser.add_argument("--seed", type=int, default=42)    #指定随机种子的参数
#     parser.add_argument("--early_stop", type=int, default=16, help='early stopping for finetune') #指定早停次数的参数
#     parser.add_argument("--froze_later", action='store_true', help='whether to froze topology in later training phase')#指定是否在后续训练阶段冻结拓扑结构的参数
...
#     except IOError as e:
#         print(f"Not exist !")
#         cfg_file = f'configs/default/{args.model}/cfg.json'   #使用默认配置文件
#
...
#             cfg = json.load(f)
#
#     args.output = str(Path(args.output) / f'{args.model}/{args.dataset}/{args.seed}')  #更新输出目录
...
#
# """Training"""
# init_score = evaluate(['test.py'])['test.py'][metric]
...
#     loss_holder.reset()
#
#     scores = evaluate(['val', 'test.py'])
#     val_score, test_score = scores['val'][metric], scores['test.py'][metric]
...
def get_training_args():
    ...
    parser.add_argument("--seed", type=int, default=42)  # 指定随机种子的参数
    parser.add_argument("--early_stop", type=int, default=16, help='early stopping for finetune')  # 指定早停次数的参数
    parser.add_argument("--froze_later", action='store_true',
                        help='whether to froze topology in later training phase')  # 指定是否在后续训练阶段冻结拓扑结构的参数
    args = parser.parse_args()
...
def record_exp(final_score, best_score, **kwargs):   #实验结果记录 （最终得分、最佳得分、可变数量关键字参数）
    results = {
        'config': vars(args),   #结果的配置
        'final': final_score,   #最终得分
        'best': best_score,     #最佳得分
        **kwargs,
    }
    exp_list = [file for file in os.listdir(args.output) if '.json' in file]
    exp_list = [int(file.split('.')[0]) for file in exp_list]
...
"""args"""
device = torch.device('cuda')
args, cfg = get_training_args()     #获取命令行参数和配置信息
seed_everything(args.seed)          #设置随机数种子
